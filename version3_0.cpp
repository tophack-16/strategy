...
void Read() {
    ...
    string strValue,sv;
    while (getline(cin,sv)) strValue = strValue + sv;
...
void TryGetGemDiff(int w, vector <int> val) { // val [0~30]
    if (10 - mygems() >= 1) {
        rep(i, 0, 5) if (Gem[i].count && val[Gem[i].color] &&  Gem[i].color != 0) {
            OPERATE o ;
            o.type = 0 ;
    ...
    if (10 - mygems() >= 2) {
        rep(i, 0, 5) rep(j, i + 1, 5) if (Gem[i].count && Gem[j].count && val[Gem[i].color] && val[Gem[j].color] && Gem[i].color != 0 && Gem[j].color != 0) {
            OPERATE o ;
            o.type = 0 ;
    ...
    if (10 - mygems() >= 3) {
        rep(i, 0, 5) rep(j, i + 1, 5) rep(k, j + 1, 5) if (Gem[i].count && Gem[j].count && val[Gem[i].color] && val[Gem[j].color] && Gem[k].count && val[Gem[k].color] && Gem[i].color != 0 && Gem[j].color != 0 && Gem[k].color != 0) {
            OPERATE o ;
            o.type = 0 ;
...
void TryGetGemSame(int w, vector <int> val) { // val [0~30]
    if (10 - mygems() >= 2) {
        rep(i, 0, 5) if (Gem[i].count > 1 && val[Gem[i].color] && Gem[i].color != 0) {
            OPERATE o ;
            o.type = 1 ;
...
int CountStatus(string name) {
    ...
    rep(id, 0, 2) if (Player[id].name == name) {
        rep(i, 0, (int) Card.size() - 1) {
            if (CANBUY(name, Card[i])) {
                res = max(res, Card[i].score) ;
            }
        }
        rep(i, 0, (int) Player[id].res.size() - 1) {
            if (CANBUY(name, Player[id].res[i])) {
                res = max(res, Player[id].res[i].score) ;
            }
        }
        rep(i, 0, (int) Noble.size() - 1) {
            if (CheckNoble(name, Noble[i])) {
                res = max(res, Noble[i].score) ;
            }
        }
    }
    return res ;
...
bool CANBLOCK(string name, int Score) {
    ...
    rep(id, 0, 2) if (Player[id].name == name) {
        rep(i, 0, (int) Card.size() - 1) {
            if (CANBUY(name, Card[i]) && Card[i].score >= Score && CANBUY(PlayerName, Card[i])) {
                OPERATE o ;
                o.type = 4 ;
                o.PurchaseCard = Card[i] ;
                if (candidate < o) candidate = o ;
